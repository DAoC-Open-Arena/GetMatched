...
queue_left
    team, reason  (unready | matched | timeout_requeue)
"""
...
def queue_entered(guild_id: int, team_name: str) -> None:
    _write("queue_entered", guild_id, team=team_name)


def queue_left(guild_id: int, team_name: str, reason: str) -> None:
    """reason: 'unready' | 'matched' | 'timeout_requeue'"""
    _write("queue_left", guild_id, team=team_name, reason=reason)

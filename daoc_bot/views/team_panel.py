...
class _IdleView(_BaseLeaderView):
    ...
    async def ready(
        ...
        event_log.queue_entered(self.guild_id, team.name)
        logger.info(
            "Guild %d: %s queued. Queue: %s",
            self.guild_id, team.name, guild_store.get_queue(self.guild_id),
        )
...
class _ReadyView(_BaseLeaderView):
    ...
    async def unready(
        ...
        event_log.queue_left(self.guild_id, team.name, reason="unready")
